...
use yii\helpers\Url;
use zrk4939\widgets\plupload\assets\PluploadAsset;
...
class Plupload extends Widget
{
    ...
    public function init()
    {
        ...
        foreach ($this->events as $event => $callback)
            $events .= "{$variableName}.bind('$event', $callback);\n";

        $this->view->registerJs("var {$variableName} = new plupload.Uploader($options);\n{$variableName}.init();\n$events");
